"""PR09 post-close reflection agent with retry and safe fallback."""
...
class ReflectionAgent(BaseAgent):
    """Highest-quality bounded reflection agent with one repair retry."""
    ...
    def run(self, input_payload: dict[str, Any], context: ToolContext | None) -> AgentResult:
        validated = ReflectionInput.model_validate(input_payload)
        ...
        fallback = ReflectionOutputFallback(
            trade_date=validated.trade_date,
            reflection_status="reflection_failed",
            fallback_action="reflection_failed",
