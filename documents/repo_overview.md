...
- PR 32 keeps public entrypoints in place while moving large private helper tails into sibling modules: `src/trading/workflows/option_strategy_builder.py` now owns the option-strategy payload builder family extracted from `trading_decision.py`, `src/trading/workflows/paper_execution_options.py` now owns the option execution/fallback helper family extracted from `paper_execution.py`, `src/web/routers/today_loaders.py` now owns the `/today` router loader/serialization helpers, and `src/web/presenters/today_workspace_detail.py`, `today_workspace_format.py`, and `today_workspace_timeline.py` now own the split helper families behind `build_ticker_workspace(...)`.
