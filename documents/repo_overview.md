...
- The historical catch-all `src/db/models/trading.py` has now been split into the `src/db/models/trading/` package, grouped by domain modules (`strategy`, `llm`, `signals`, `macro_calendar`, `risk`, `execution`, `intraday`, `reflection`, `universe`) while preserving the `src.db.models.trading` import surface through package re-exports.
...
- PR 32 keeps public entrypoints in place while moving large private helper tails into sibling modules: `src/trading/workflows/option_strategy_builder.py` now owns the option-strategy payload builder family extracted from `trading_decision.py`, `src/trading/workflows/paper_execution_options.py` now owns the option execution/fallback helper family extracted from `paper_execution.py`, and `src/web/presenters/today_workspace_detail.py`, `today_workspace_format.py`, and `today_workspace_timeline.py` now own the split helper families behind `build_ticker_workspace(...)`.
- PR 33 continues that split on the `/today` surface: `src/web/routers/today_loaders.py` is now a thin compatibility hub over `src/web/routers/loaders/` section modules (`header_system`, `portfolio`, `candidates`, `risk_macro`, `universe_learning`, `trades`, `ticker_detail`, `_format`), preserving the existing `today.py` monkeypatch seam while removing the old loader god file.

- PR 31 cleanup removes the dead persistence-only `learning_factor_applications` and `macro_readthrough_events` DB models/tables, but intentionally keeps `HistoricalReplayRun` plus `candidate_outcome_evaluations.historical_replay_run_id` so the replay persistence path can be wired later without reintroducing the schema anchor.
- PR 31 also turns `src/trading/repositories/sqlalchemy.py` into a thin facade over `src/trading/repositories/_base.py` and domain mixins under `src/trading/repositories/mixins/`, preserving the `SQLAlchemyTradingRepository` and `SqlAlchemyTradingRepository` public API while separating strategy, signals, risk, execution, intraday, reflection, macro-calendar, and runtime-misc responsibilities.
- `src/web/routers/today.py` now also carries the latest PR 11 redesign slice on the `/today` workstation: portfolio analytics derived from persisted equity history, a collapsed attention feed, trade-plan / bull-bear / signal-group evidence on Trades, a split market-vs-earnings risk calendar, and a candidate evaluation timeline, all still rendered server-side through FastAPI + Jinja.
- PR 14a adds a thin persisted runtime observability contract for live preopen runs. `src/db/models/trading.py` now defines `TradingRuntimeRun`, Alembic revision `027_trading_runtime_runs.py` creates the `trading_runtime_runs` table plus latest-by-phase / latest-by-phase-trade-date indexes, `src/trading/repositories/sqlalchemy.py` can save and reload normalized runtime rows, and `src/trading/runtime/preopen.py` plus `scripts/run_trading_live_preopen_order_smoke.py` now persist successful or failed preopen runtime summaries at the session-owning entrypoints instead of making `/today` reverse-engineer the morning funnel from downstream decision tables. `src/web/routers/today.py`, `src/web/presenters/today_overview.py`, and `src/templates/today.html` now surface the latest same-trade-date persisted `preopen` row in `Overview -> Latest Preopen Run`, including status, execution mode, timestamps, and funnel counts or an explicit unavailable state when no persisted row exists yet.
...
- Alembic revision `027` adds `trading_runtime_runs`, a generic persisted scheduler/runtime report table keyed by phase/trade-date/recency for operator observability and later multi-phase reuse.
...
- Live preopen wiring now consumes active and shadow learning factors through `src/trading/learning/apply.py`: strategy-scoped active `increase_score` factors can boost deterministic matcher scores, portfolio/risk tightening factors can shrink the preopen macro risk budget, shadow factors remain audit-only, and `strategy_proposals` now carry a nullable `daily_reflection_id` lineage link via Alembic revision `026`.
- The same live preopen dependency assembly now also folds active research watchlist tickers into the loaded `UniverseFilterConfig.manual_include` set before morning universe scan, so `/watchlist` symbols enter the next preopen candidate universe immediately while still flowing through the ordinary `scanner` provenance path rather than a separate `watchlist_pin` channel.
- `/today` `Learning & Strategies` now includes a read-only observability block driven by `src/web/presenters/today_learning_strategies.py`, showing the current loop funnel from learning factors through proposals/definitions/promotions plus the active learning factors that can presently influence strategy score or risk-budget behavior.
...
- `src/templates/today.html` now renders the 5-tab information architecture from the implementation plan: `Portfolio`, `Trades`, `Candidates`, `Risk & Macro`, and a muted right-aligned `System` tab. The header is now a compact KPI bar plus context line, portfolio is the summary-first home with per-table aggregate lines and a collapsed needs-attention empty state, and `System` merges the old learning/ops surfaces. The tab bodies are now split into bounded partials under `src/templates/today/` so the main template remains a thin tab shell instead of another large monolith.
...
- `src/web/presenters/today_portfolio_analytics.py` and `src/web/presenters/signal_evidence.py` now supply the portfolio-performance visualization and grouped signal evidence used by the redesigned `/today` trades and portfolio surfaces
...
- the smoke script now reuses an already-active manual request for the target ticker when one exists, so operator reruns no longer fail immediately on `uq_manual_ticker_requests_active_ticker`; only smoke-created requests are auto-dismissed during cleanup
- `src/web/presenters/today_overview.py` now treats `risk_macro.availability` as the canonical health source for `Overview` live-status and system-issue rendering, and it formats `Gross Exposure` defensively so large persisted exposure values are not multiplied into absurd percentages
