...
- Added trade identity requirements for portfolio pools: core holdings, tactical stock trades, tactical option income, RiskManager hedge overlays, and watch-only candidates.
...
- Refined trade identity taxonomy into portfolio-pool identities: `core_holding`, `tactical_stock_trade`, `tactical_option_income`, `risk_hedge_overlay`, and `watch_only`; strategy expression buckets remain the alpha/expression layer, and risk hedge overlays are RiskManager-owned paper option actions.

## PR Slice Status

| Slice | Scope | Status | Notes |
| --- | --- | --- | --- |
| PR 1 | Trading foundation schema + strategy catalog | Pending | Adds 15 tactical strategies, 4 expression buckets, manual ticker request schema, prompt registry/schema, and portfolio-pool trade identity taxonomy. |
...
| PR 6 | Paper options strategy layer + assignment risk | Pending | Paper/simulation-only short puts and RiskManager-owned hedge overlays; sell/close/roll/avoid/assignment plan plus worst-case assignment checks. |
