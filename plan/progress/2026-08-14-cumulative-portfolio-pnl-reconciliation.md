...
- 2026-08-22: Investigated the post-deploy zero-P&L screenshot. The deployed web/scheduler containers contained the reconciled code, but were created at `2026-08-21T23:08Z`, while the latest displayed snapshot was written by the old scheduler at `2026-08-21T19:00Z` with empty metadata and zero P&L. After the user approved the renewed exact report, production `--apply` repaired that one latest snapshot and 2 newer sell cash effects. The post-apply dry-run returned zero snapshot/cash repairs with no mismatches or validation errors. Server-rendered HTML for all six Today tabs now consistently returns realized `-$14,268.27` and unrealized `-$4,115.74`; an end-user browser refresh/screenshot remains the final visual check.

## Final Results

Production data and server-rendered Today values are corrected. Final end-user visual refresh remains pending.
