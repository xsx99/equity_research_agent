...
def test_option_risk_manager_blocks_event_through_expiry_credit_spread_when_policy_forbids():
    ...
    assert assessment.status == "rejected"
    assert assessment.reason_code == "event_through_expiry_short_premium_blocked"
...
def test_option_risk_manager_blocks_assignment_when_sector_concentration_after_assignment_is_too_high():
    portfolio = PortfolioContext(
        ...
        positions=(
            PortfolioPosition(
                ...
                event_type=None,
                macro_sensitivity="ai_capex",
                margin_requirement=12_500,
            ),
        ),
        open_strategy_exposure={},
        current_factor_exposure=(),
        stock_margin_requirement=12_500,
        option_margin_requirement=0.0,
        total_margin_requirement=12_500,
    )
    config = RiskConfigResolver().resolve(
        risk_appetite=RiskAppetiteProfile.BALANCED,
        portfolio_context=portfolio,
    ...
    assert assessment.status == "rejected"
    assert assessment.reason_code == "assignment_sector_concentration_cap"
